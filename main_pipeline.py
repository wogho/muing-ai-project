...
def run_stem_separation(input_file, output_dir):
    ...
    print("\n>>> 1. 스템 분리를 시작합니다.")
    command = f"demucs -j 1 --shifts 0 --segment 7 \"{input_file}\" -o \"{output_dir}\""
    try:
        # check=True를 사용하여 오류가 발생하면 예외를 발생시킵니다.
        subprocess.run(command, shell=True, check=True, capture_output=True, text=True)
        print(">>> 스템 분리가 완료되었습니다.")
    except subprocess.CalledProcessError as e:
        # Demucs 실행 중 발생한 오류를 명확하게 출력합니다.
...
def extract_melody(input_file, output_dir):
    ...
    if not os.path.exists(vocals_path):
        raise FileNotFoundError("보컬 파일(vocals.wav)을 찾을 수 없습니다.")
    ...
    # Basic-Pitch가 생성한 파일들의 이름을 표준화합니다.
...
# extract_chords, extract_rhythm, integrate_data 함수들은 기존과 동일하게 유지됩니다.
# ... (생략된 기존 함수들) ...
...
def run_full_pipeline(input_file, output_dir, chord_source):
    """
    오디오 파일을 분석하는 전체 파이프라인.
    성공 또는 실패 시 status.json 파일을 생성합니다.
    """
    try:
        ...
        for stem in stems:
            original_path = os.path.join(demucs_output_dir, f'{stem}.wav')
            if os.path.exists(original_path):
                # 파일을 최종 목적지로 옮깁니다.
                final_path = os.path.join(output_dir, f'{stem}.wav')
                shutil.move(original_path, final_path)
                # 클라이언트가 사용할 파일명만 저장합니다.
                result_audio_files[stem] = f'{stem}.wav'
        
        # Demucs가 남긴 빈 폴더를 정리합니다.
        ...
        # 3. 성공 상태를 status.json에 기록합니다.
        status_data = {
            'status': 'complete',
            'message': '스템 분리가 완료되었습니다.',
        ...
        # 4. 오류 발생 시, 오류 상태를 status.json에 기록합니다.
